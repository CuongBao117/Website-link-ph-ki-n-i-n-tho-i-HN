-- Migration 019: ẩn/hiện sản phẩm TẠM THỜI (không xoá) — sản phẩm ẩn không hiện ở trang chủ,
-- danh mục, tìm kiếm, "sản phẩm liên quan", sitemap, và không đặt được (place_order() chặn luôn,
-- phòng trường hợp khách có giỏ hàng cũ từ trước khi ẩn, hoặc link sản phẩm đã lỡ chia sẻ nơi
-- khác) — nhưng vẫn còn nguyên trong admin để bật hiện lại bất kỳ lúc nào.
...
-- Cập nhật place_order() — thêm chặn sản phẩm đang ẩn, y hệt cách OUT_OF_STOCK đã chặn (xem
-- migration_006/010/016 cho lịch sử các lần sửa hàm này).
...
begin
  ...
  loop
    ...
    if v_product.is_hidden then
      raise exception 'PRODUCT_HIDDEN' using detail = jsonb_build_object(
        'slug', v_product.slug, 'name', v_product.name
      )::text;
    end if;
