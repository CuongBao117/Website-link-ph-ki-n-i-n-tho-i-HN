...
// có giá khác nhau, trả về cả "price" (thấp nhất, dùng cho SEO/schema) lẫn "min"/"max" kèm cờ
// "isRange" để nơi hiển thị tự ghép thành "40.000đ-65.000đ".
export function getDisplayPrice(product) {
  const options = product?.priceOptions || [];
  if (options.length === 0) return { price: product?.price ?? null, min: null, max: null, isRange: false };
  const prices = options.map((o) => o.price).filter((p) => Number.isFinite(p));
  if (prices.length === 0) return { price: product?.price ?? null, min: null, max: null, isRange: false };
  const min = Math.min(...prices);
  const max = Math.max(...prices);
  return { price: min, min, max, isRange: min !== max };
}
